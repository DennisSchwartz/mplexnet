...
var createTensor = function (network) {
    ...
    network.get('layers').each(function (l) { // for each layer
        ...
        var e = network.get('edges').byLayer(l, network); // get all edges in current layer
        e.each(function (e) {
    ...
    //var m = require('mathjs').create().zeros(sizeV,sizeV);
...
//TODO: Try to create the tensor using this
...
var kronecker_product = function (a, b)
{
    ...
    var res = math.zeros(colsA*colsB,rowsA*rowsB, 'sparse');
    for (var i=0;i<rowsA;i++) {
        for (var j=0;j<colsA;j++) {
            ...
            res = math.subset(res, indices, block);
        }
    }
    return res;
...
var blkdiag = function (nodeTensors, size) {
    // create block diagonal matrix from nodeTensors
    //var n = 0, m = 0;
    ...
    //            var temp = [];
    //            if (j===i) {
    //                temp = temp.concat(nodeTensors[i][k]);
    //                //console.log(nodeTensors[i][k]);
    //            } else {
    //                temp = temp.concat(math.zeros(size));
    //                //console.log(math.zeros(size));
    //            }
    //        }
    //        res[i][k] = temp;
    //    }
    ...
    //    var line = [];//test[i].concat(math.zeros(matSize - test[i].length));
    //    for (var j=0;j<l;j++) {
    //        console.log(j, i, nodeTensors[j][i % size]);
    //        if (j===l%i) {
    //            line = line.concat(nodeTensors[j][i % size]);
    //            console.log('Hello: ' + line);
    //        } else {
    //            line = line.concat(math.zeros(size));
    //        }
    //    }
    //    console.log('line at ' + i + ': ' + line);
    //    res.push(line);
    //}
    //for (var i=0;i<l;i++) {
    //    res = res.concat(math.zeros(size, size));
    //}
    return res;
