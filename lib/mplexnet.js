...
var Network = Backbone.Model.extend({
    ...
    initialize: function(input) {
        ...
        if (Options.inputFiles === 'multiple') {
            ...
            this.parseInput(input.data, Options.inputFiles);
        }
    },
    parseInput: function (data, format) {
        ...
        if (format === 'xgmml') {
            ...
            for (var e in graph.childNodes) {
                ...
                if (element.nodeName === 'edge') {
                    ...
                    var source = nodes.get(res['source']);
                    if (!source) {
                        source = new Node(res['source']);
                        nodes.add(source);
                    }
                    var target = nodes.get(res['target']);
                    if (!target) {
                    ...
                    var sourceID = res['source'] + res['sourcelayer'];
                    var targetID = res['target'] + res['targetlayer'];
