...
var EdgeCol = Backbone.Collection.extend({
    ...
    byV: function (id, net) {
        var filtered = this.filter(function (edge) {
            return net.get('nodes').get(edge.get('source')).get('node').get('id') === id || net.get('nodes').get(edge.get('target')).get('node').get('id') === id;
        });
        return new EdgeCol(filtered);
    },
...
var Network = Backbone.Model.extend({
    ...
    initialize: function(input) {
        ...
        var data = input.data;
        if (input.edges) {
            data = {nodes: input.nodes, edges: input.edges, aspects: input.aspects };
        }

        /*
            Automatic detection of input file format
         */
        if (Options.loglevel > 0) console.log('The input format was specified as: ' + Options.inputFormat + ".");
        var guessedFormat = guessFormat(data);
        if (Options.loglevel > 0) console.log('Automatic detection found: ' + guessedFormat + ".");
        if (Options.inputFormat !== guessedFormat) {
            console.warn('Detected and specified formats differ!');
            this.trigger('Detected different data format than specified!', guessedFormat);
        }
        if (guessedFormat === 'error' && !Options.inputFormat) throw "There was an error detecting the format of the input data!";
        Options.inputFormat = guessedFormat;

        /*
            Start creation of network
         */
        this.createNetwork(data, Options.inputFormat);
    },
    createNetwork: function (data, format) {
        ...
        if (format === 'xgmml') {
            ...
            // TODO: Allow for different labels in format
...
var guessFormat = function (data) {
    ...
    if (typeof data === 'string') {
        ...
        if (lines[0].trim() === '<?xml version="1.0"?>' &&
            lines[1].trim() === '<!DOCTYPE graph SYSTEM "http://www.cs.rpi.edu/~puninj/XGMML/xgmml.dtd">') {
            return 'xgmml'
        } else if (lines[0].split(Options.inputFileDelimiter).length > 1 &&
            lines[0].indexOf(Options.sourceFieldLabel) > -1 &&
            lines[0].indexOf(Options.targetFieldLabel) > -1 ) {
            return 'csv'
        } else {
            return 'error'
        }
